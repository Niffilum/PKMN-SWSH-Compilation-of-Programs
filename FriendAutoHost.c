...
static const Command autoHost[] = {
	...
	{NOTHING, 1},
	{A, 1},				
	{NOTHING, 400},	// Wait until game starting showing abilities (if any has one)
	
	// add ONE friend request 24,51
	{HOME, 1},      {NOTHING, 15},
	{UP, 1},        {NOTHING, 55},
	{A, 5},         {NOTHING, 155},
	{A, 5},         {NOTHING, 105},
	...
	if (commandIndex == -1)
	{
		...
		else //if (m_sequence <= 13)
		{
			...
			if (m_sequence % 3 == 0) // 3,6,9,12
			{
				...
				{
					commandIndex = 74 + (number / 3 - 1) * 2;
					m_endIndex = 81;
				}
				else // 1,4,7,2,5,8
				{
					commandIndex = 72 + (number / 3) * 2;
					m_endIndex = (number % 3 == 1) ? 79 : 77;
				}
			}
